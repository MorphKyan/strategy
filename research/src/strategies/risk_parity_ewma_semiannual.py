...
def run_strategy(df: pd.DataFrame, config: dict):
    """
    Risk parity variant that combines EWMA volatility estimates with
    a semiannual rebalance calendar to reduce turnover pressure.
    """
    ...
    semiannual_ends = df.index.to_series().resample("2QE").max().tolist()
    ...
    for i, (date, row_rets) in enumerate(returns_filled.iterrows()):
        ...
        if is_opened:
            ...
            is_rebalance_day = date in semiannual_ends
