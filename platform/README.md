...
- `scripts/run_live_cycle.py`: live-mirror portfolio entry — `reconcile` overwrites state from real holdings, `plan` renders the next-day order ticket.
...
- `src/platform_core/`: platform engine package.
  ...
  - `live.py`: live-mirror portfolio (mark-to-real reconcile + dry-run order-ticket planning; never simulates fills).
