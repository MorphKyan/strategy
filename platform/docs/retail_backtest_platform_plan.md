...
当前仓库在 `research/` 下已经具备：
- YAML 配置驱动的单策略回测入口：`research/main.py`。
- 本地 CSV 数据加载与自动补数据：`research/src/data_handler.py`。
- 简单组合再平衡引擎：`research/src/engine.py`。
- 策略模块动态加载：`research/src/strategies/<strategy>.py`。
- 风险平价研究报告与实验归档：`research/scripts/run_experiment.py`、`research/reports/experiments/`。
...
推荐用事件驱动日频引擎，而不是完全向量化引擎作为主引擎。回测底层的估值、交易执行完全采用**无复权价格**（原始价格），消除了复权价格带来的隐式复利与现金拖累，从而提供高精度的业绩再现。

每日流程：
1. 加载当日可见原始数据，并自动根据侧车后复权因子合成 `adj_close` 供策略计算信号。
2. 盘前更新证券事件：
   - 拆分折算：若为折算日，按拆分比例折算持仓股数与成本价。
   - 除息登记：若为除息日，将应收股利登记入账（应收分红计入总资产净值）。
   - 资金到账：若为派息日，将应收分红划拨可用现金账户，模拟现金Lag延迟。
3. 更新持仓市值、可用现金等状态。
4. 执行到期冷却池清理。
5. 根据策略时间表选择当前策略版本。
6. 策略根据平滑复权价格产生目标组合或订单意图。
7. 风控与交易规则校验（基于无复权原始价限制 lot_size）。
8. 撮合成交。
9. 未成交订单按规则进入 `retry_next_day`、`cancel` 或 `mark_failed`。
10. 生成 checkpoint。
11. 写入日志、净值、交易和持仓。
...
任务：
- 平台文档、源码、配置、结果集中在 `platform/`。
- 当前 `research/src/strategies/risk_parity.py` 保持不动。
