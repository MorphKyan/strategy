...
| **global_ewma** | baseline | 3.99% | -3.08% | **1.552** | 47.90% | 53.0 |
| | candidate | 3.99% | -4.44% | 1.487 | 61.04% | 53.0 |
| **global_dividend_ewma** | baseline | 3.79% | -2.90% | **1.531** | 46.99% | 37.0 |
...
| **us_blend_ewma** | baseline | 4.07% | -3.29% | **1.486** | 56.63% | 57.0 |
...
### 4.1 绩效对比归因
1. **等权与局部配置的改进**：
   在标的集中的等权配置组（`baseline_mvp_equal_weight`）和国内纳指全天候（`global_nasdaq_all_weather_ewma`）下，新策略的 Sharpe 比率实现了显著的跃升（如等权组夏普由 1.938 提升至 2.756），且最大回撤大幅收紧。这是因为 GARCH 能及时捕捉这些标的的波动时变特征，降低了组合的风险暴露。
2. **大类与境外配置的全线性能退化**：
