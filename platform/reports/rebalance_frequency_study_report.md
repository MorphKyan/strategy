...
2. **例外情况：全球多资产配置策略更适合“日调仓”**
   在包含全球多资产（美股标普/纳指、德股、商品豆粕/能化、黄金及国债）的配置策略中（如 `baseline_r2_global_ewma` 和 `baseline_r2_global_dividend_ewma`），**日调仓 (Daily)** 取得了**更好的夏普比率和更低的最大回撤**。这是因为全球弱相关资产在每日频度下的风险平价计算能提供更精细的风险分散和波动率控制，其超额收益足以覆盖更高的日内交易摩擦。
...
2. **跨区域/多大类资产全球配置组合 (推荐: Daily)**
   * *典型配置*：`baseline_r2_global_dividend_ewma`, `baseline_r2_global_ewma`
