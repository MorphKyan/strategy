...
class QDIIPremiumFactorRotationRiskParityStrategy(RiskParityLWCovStrategy):
    """
    基于折溢价风险控制与多因子打分的 QDII ETF 轮动风险平价策略 (QDIIPremiumFactorRotationRiskParityStrategy)
    
    该策略具有以下几大核心设计：
    1. 动态折溢价监控与防御机制 (Premium Risk Control)：
       - 对于在境外时差交易中可能产生高溢价的 QDII 资产，计算其当前交易溢价率：
         Premium_t = Close_t / NAV_t - 1。
         （若无 nav 数据，则默认为 0）。
       - 引入参数 premium_upper_limit (默认 0.015)。若溢价率超过该阈值，则直接判定为高溢价风险，
         在轮动候选池中将该资产直接剔除（或综合得分设为负无穷），避免买在溢价高点导致未来均值回归的折返损耗。
    2. 多因子打分轮动筛选 (Multi-factor Scoring & Rotation)：
       - 针对不同 sleeve (如 hs300, bond, gold, commodity, qdii 等) 内部 of 资产进行多因子打分。
       - 因子包含：
         a) 动量因子 (Momentum)：滚动 N_m 日的资产收益率（对于 QDII/跨境资产使用 NAV，防止二级市场情绪噪声扭曲，其余资产使用 Close）。
         b) 波动率因子 (Volatility)：滚动 N_v 日日收益率标准差的相反数（低波得分高）。
         c) 折溢价因子 (Premium)：N_p 日平均溢价率的相反数（低溢价或折价得分高，非 QDII 资产设为 0）。
       - 因子打分前在 sleeve 内部进行横截面 Z-score 标准化，加权得到综合得分。
       - 轮动时在每个 sleeve 内部选出综合得分排名前 top_k_per_sleeve (默认 1) 的资产。
       - 为抑制 whipsaw 频繁替换，对上一期已入选的资产赋予得分偏置 switching_threshold (默认 0.05)。
    3. 全天候风险平价配置 (Risk Parity Allocation)：
       - 仅对每个 sleeve 筛选出的资产，计算其 Ledoit-Wolf 协方差，并求解风险等贡献权重。
    4. 调仓平滑与阻尼过渡 (Weight Damping)：
       - 当持仓发生轮动或绝对偏差超过 rebalance_threshold 时触发调仓。
       - 调仓时采用 damping_factor (默认 1.0) 平滑最终权重，削减交易换手率与印花税损耗。
    """
    name = "qdii_premium_factor_rotation_risk_parity"
    version = "0.1.0"

    def initialize(self, context: StrategyContext) -> None:
        context.runtime["opened"] = context.params.get("init_mode", "calculate") == "manual"
        context.runtime["rebalance_frequency"] = context.params.get("rebalance_frequency", "daily")
        context.runtime["selected_assets"] = []  # 记录上一期入选的资产
    ...
    def _inverse_vol_target(self, context: StrategyContext, universe: list[str]) -> TargetPortfolio | None:
        ...
        top_k = int(context.params.get("top_k_per_sleeve", 1))
        switching_threshold = float(context.params.get("switching_threshold", 0.05))
        sleeve_mapping = context.params.get("sleeve_mapping", {})
        premium_upper_limit = float(context.params.get("premium_upper_limit", 0.015))
        
        # 因子权重
        factor_weights = context.params.get("factors", {"momentum": 0.4, "volatility": 0.3, "premium": 0.3})
        w_mom = float(factor_weights.get("momentum", 0.4))
        w_vol = float(factor_weights.get("volatility", 0.3))
        w_prem = float(factor_weights.get("premium", 0.3))
        
        # 因子滚动窗口
        n_mom = int(context.params.get("momentum_window", 20))
        n_vol = int(context.params.get("volatility_window", 60))
        n_prem = int(context.params.get("premium_window", 20))
        ...
        for asset_id in universe:
            ...
            if "nav" in hist.columns:
                navs[asset_id] = hist["nav"]
            elif "acc_nav" in hist.columns:
                navs[asset_id] = hist["acc_nav"]
            else:
                navs[asset_id] = pd.Series(dtype=float)
        ...
        for asset_id in universe:
            ...
            is_qdii = sleeve_mapping.get(asset_id) == "qdii"
            
            # 动量价格序列选择
            if is_qdii and "acc_nav" in hist.columns:
                price_series = hist["acc_nav"]
            ...
            if len(price_series) > n_mom:
                mom_values[asset_id] = float(price_series.iloc[-1] / price_series.iloc[-n_mom - 1] - 1.0)
            else:
                mom_values[asset_id] = 0.0
                
            # 波动率计算
            if len(price_series) > n_vol:
                daily_ret = price_series.pct_change().dropna().tail(n_vol)
                vol_values[asset_id] = -float(daily_ret.std())
            else:
                vol_values[asset_id] = 0.0

            # 折溢价计算
            has_nav = "nav" in hist.columns or "acc_nav" in hist.columns
            if has_nav:
                close_hist = hist["close"]
                nav_hist = hist["nav"] if "nav" in hist.columns else hist["acc_nav"]
                premium_series = (close_hist / nav_hist - 1.0).dropna()
                if not premium_series.empty:
                    prem_current[asset_id] = float(premium_series.iloc[-1])
                    prem_values[asset_id] = -float(premium_series.tail(n_prem).mean())
                else:
                    prem_current[asset_id] = 0.0
                    prem_values[asset_id] = 0.0
            else:
                prem_current[asset_id] = 0.0
                prem_values[asset_id] = 0.0

        # 3. 按 sleeve 分组，在各 sleeve 内进行多因子打分与折溢价控制筛选
        last_selected = context.runtime.get("selected_assets", [])
        
        categories = {}
        for asset_id in universe:
            cat = sleeve_mapping.get(asset_id, "default_sleeve")
            categories.setdefault(cat, []).append(asset_id)
            
        selected_assets = []
        for cat, assets_in_sleeve in categories.items():
            valid_assets = []
            for asset_id in assets_in_sleeve:
                # 只有 qdii 资产才执行高溢价风控剔除
                if sleeve_mapping.get(asset_id) == "qdii" and prem_current.get(asset_id, 0.0) > premium_upper_limit:
                    continue
            ...
            if len(valid_assets) <= 1:
                selected_assets.extend(valid_assets)
                continue
                
            sleeve_moms = np.array([mom_values[a] for a in valid_assets])
            sleeve_vols = np.array([vol_values[a] for a in valid_assets])
            sleeve_prems = np.array([prem_values[a] for a in valid_assets])
            ...
            z_moms = zscore(sleeve_moms)
            z_vols = zscore(sleeve_vols)
            z_prems = zscore(sleeve_prems)
            
            scores = {}
            for idx, asset_id in enumerate(valid_assets):
                score = w_mom * z_moms[idx] + w_vol * z_vols[idx] + w_prem * z_prems[idx]
                if asset_id in last_selected:
                    score += switching_threshold
                scores[asset_id] = score
                
            sorted_assets = sorted(valid_assets, key=lambda a: scores[a], reverse=True)
            chosen = sorted_assets[:min(top_k, len(sorted_assets))]
            selected_assets.extend(chosen)
            
        context.runtime["selected_assets"] = selected_assets
        
        if not selected_assets:
            return None
            
        # 4. 对选出的资产进行 Ledoit-Wolf 协方差估计并求解风险平价权重
        closes_sub = {}
        ...
        Sigma_sub = self._estimate_covariance(X, shrinkage_target)
        try:
            sub_weights = self._solve_risk_parity(Sigma_sub)
        except Exception:
            sub_weights = np.ones(N) / N
            
        weights_full = {asset_id: 0.0 for asset_id in universe}
        for idx, asset_id in enumerate(selected_assets):
            weights_full[asset_id] = float(sub_weights[idx])
            
        return TargetPortfolio(weights_full)
...
BUILTIN_STRATEGIES: dict[str, type[Strategy]] = {
    ...
    QDIIPremiumFactorRotationRiskParityStrategy.name: QDIIPremiumFactorRotationRiskParityStrategy,
}
