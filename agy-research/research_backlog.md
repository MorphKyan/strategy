...
| :--- | :--- | :--- | :--- | :--- | :--- |
| R011 | 基于下行半协方差 (Downside Semi-Covariance) 的稳健风险平价策略 | 传统风险平价假设资产收益的上下行对称性，无法刻画市场下跌时资产相关性陡增的现象。通过下行半协方差重构协方差矩阵，期望降低资产共震下跌时的极端下行回撤。 | 中-高 | 日线行情及复权净值数据 | `baseline_risk_parity_lw_cov.yaml` |
| R012 | 基于 CVaR 动态预算与波动率目标控制的风险平价策略 | 传统风险平价组合波动随成分股波动同步放大，且遭遇黑天鹅时尾部回撤严重。结合动态波动率目标控制与 CVaR 动态风险预算调整，动态引入现金或高低风险资产预算微调，期望能显著平抑极端市场尾部回撤。 | 中 | 日线行情及复权净值数据 | `baseline_r3_global_nasdaq_all_weather_ewma.yaml` |
| R013 | 基于 `etf_selection` 多维打分筛选的 ETF 轮动风险平价策略 | 当前风险平价多针对固定标的池。该课题利用打分系统（流动性、动量、估值等因子）对中国宽基及行业 ETF 动态筛选出 Top-K 标的池，再输入风险平价或 HRP 算法，期望融合轮动阿尔法与风险防御。 | 高 | 日线行情及基本面指标数据 | `baseline_m3m4_fundamental.yaml` |
