...
| :--- | :--- | :--- | :--- | :--- | :--- |
| R027 | 基于收缩指数加权移动平均 (Shrunk EWMA) 协方差估计的稳健风险平价策略 | 结合 EWMA 的时变相关性响应与 Ledoit-Wolf 收缩的数值稳定性，解决纯 EWMA 在短窗口下条件数过大、权重抖动与调仓磨损严重的问题，以降低换手率并提升多资产组合在波动市场下的夏普比率。 | Medium | 本地 ETF 历史行情数据 | `baseline_r1_domestic_rolling.yaml`, `baseline_r3_global_nasdaq_all_weather_ewma.yaml` |
| R028 | 基于非对称磁滞区间与系统波动触发的换手约束风险平价策略 | 使用非对称磁滞区间控制调仓触发，小幅波动不调仓。调仓阈值与系统波动率负相关：低波时放宽阈值抑制 whipsaw 调仓磨损，高波时收窄阈值确保风险快速对齐与防御响应，以实现低换手率与及时风险控制的平衡。 | Medium | 本地 ETF 历史行情数据 | `baseline_mvp_equal_weight.yaml`, `baseline_r1_domestic_rolling.yaml` |
