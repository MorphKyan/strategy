...
| :--- | :--- | :--- | :--- | :--- | :--- |
| R004 | 基于 GARCH(1,1) 与下行半方差的非对称风险平价策略 | 传统风险平价同等对待向上/向下波动，但仅有下行波动会导致真实回撤。拟结合 GARCH 捕获时变波动率，并以半方差（Semi-variance）替代传统方差，仅分配下行风险预算，在防范回撤的同时释放上行收益潜力。 | High | 各 ETF 历史日度收盘价数据 | `platform/configs/baseline_r1_domestic_rolling.yaml` |
