...
| :--- | :--- | :--- | :--- | :--- | :--- |
| R025 | 基于 Huber M-估计稳健协方差与自适应收缩的风险平价策略 | 金融收益率常有肥尾和极端值，传统协方差易失真导致调仓剧烈。本研究引入 Huber M-估计对极端异常值进行降权，结合常数相关收缩目标以提高协方差稳定性。详见后文详细说明。 | 中高 | 历史市场日线及复权净值数据 | platform/configs/ 下的 baseline_r1_domestic_rolling.yaml 和 baseline_r3_global_nasdaq_all_weather_ewma.yaml |
| R026 | 基于 L2 正则化权重偏差惩罚与交易成本感知的换手优化风险平价策略 | 在目标函数中加入与当前持仓权重偏差的 L2 正则化惩罚项（Ridge 惩罚），平滑调仓路径以避免临界点频繁微调（whipsaw）。相比 L1 约束可规避角点解，更有效降低换手。详见后文详细说明。 | 高 | 历史市场数据，当前持仓状态 | platform/configs/ 下的 baseline_r1_domestic_rolling.yaml 等多资产配置 |
