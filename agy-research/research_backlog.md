...
| :--- | :--- | :--- | :--- | :--- | :--- |
| R029 | 基于独立成分分析 (ICA) 与独立成分风险平价 (ICRP) 的非正态多资产配置策略 | 提取非正态/高偏肥尾特征下的统计独立源，在 ICA 独立成分空间进行风险预算分配，期望在尾部联动极端行情中提供更佳下行避险。 | Medium-High | 多资产日频 ETF 价格数据（包含国内、跨境、债券与商品 ETF） | `baseline_r2_global_ewma.yaml`<br>`baseline_r3_global_nasdaq_all_weather_ewma.yaml`<br>`baseline_risk_parity_lw_cov.yaml` |
| R030 | 基于非线性收缩 (Non-linear Shrinkage) 协方差估计的稳健风险平价策略 | 引入基于随机矩阵理论的非线性收缩协方差估计器校正样本特征值失真，期望在高维多资产 ETF 组合中提供更精确的风险预算，提升夏普比并降低年化换手率。 | High | 日频多资产及多行业 ETF 价格数据 | `baseline_risk_parity_lw_cov.yaml`<br>`baseline_r2_global_ewma.yaml`<br>`baseline_r7_cluster_representative_damped.yaml` |
| R031 | 结合宏观状态识别与 GARCH-MIDAS 混频波动率估计的动态风险预算策略 | 引入月频/季频宏观基本面因子（如PMI、利差）与日频价格混频联立估计长期波动率，前瞻性动态调整资产风险预算，以期在宏观周期拐点实现更优的夏普提升与防守。 | Medium | 日频 ETF 价格数据，低频（月/季频）中国及全球宏观数据（PMI、信用利差、十年期国债收益率等） | `baseline_r1_domestic_rolling.yaml`<br>`baseline_r2_global_ewma.yaml`<br>`baseline_r3_global_nasdaq_all_weather_ewma.yaml` |
...
| :--- | :--- | :--- | :--- |
| R027 | 基于收缩指数加权移动平均 (Shrunk EWMA) 协方差估计的稳健风险平价策略 | 0cfeb6cc-fed3-490f-adda-5929f7c75ed4 | 在 13 个符合 3 年历史的平台基准配置测试下表现退化严重：IS Sharpe 发生明显下降（如国内 EWMA 夏普从 1.575 退化至 1.016，全球 EWMA 从 1.285 退化至 0.891，最大回撤显著放大）。同时，起点敏感性测试显示极度不稳定，Sharpe 波动标准差在 0.53 ~ 0.67 之间，严重突破合格线（<0.25），存在明显的样本过拟合风险。因未通过验收门槛，判定为 Failed，物理拒绝合入平台，策略代码与注册信息已完全擦除。 |
