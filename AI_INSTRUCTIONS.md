...
## 研究验证硬性要求
- **固定测试样本**：`2025-07-01`（含）之后的数据固定为最终测试样本；`2025-06-30`（含）之前的数据为训练/研究样本。
- **禁止测试样本泄漏**：研究阶段不得使用最终测试样本进行策略构思、参数选择、ETF 组合筛选、候选淘汰、阈值设定、缓存复用或报告推荐。
- **起点敏感性测试**：所有平台策略或 ETF 组合研究在提交前，必须从最早可用共同交易日起，每隔 2 个自然月生成一个 `start_date`，在训练/研究样本内逐一起跑回测，验证起点变化是否显著影响 Sharpe、最大回撤、换手、成交笔数、拒单数和候选相对 baseline 的排序。
- **最终测试阶段**：只有当训练/研究样本内多重对照和起点敏感性测试通过后，才允许冻结策略/组合并使用 `2025-07-01`（含）之后数据做最终测试。看到最终测试结果后不得再改策略、参数、ETF 成分、调仓规则或验收阈值。
- **提交门槛**：只有训练样本稳定、起点敏感性稳定，并且最终测试样本中仍表现良好的研究，才允许提交为可合入成果；否则必须标记为 Failed 或仅研究观察。
